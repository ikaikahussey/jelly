...
interface AuthContextType {
  ...
  login: (provider: 'google' | 'github' | 'apple', redirectUrl?: string) => void;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [authProviders, setAuthProviders] = useState<{ google: boolean; github: boolean; apple: boolean; email: boolean; } | null>(null);
  ...
  const fetchAuthProviders = useCallback(async () => {
    try {
      ...
      setAuthProviders({ google: false, github: false, apple: false, email: true });
      setHasOAuth(false);
  ...
  // OAuth login method with redirect support
  const login = useCallback((provider: 'google' | 'github' | 'apple', redirectUrl?: string) => {
