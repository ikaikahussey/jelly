...
export function NewBlogPost({ onCreated, onCancel }: NewBlogPostProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-koa-200 p-4 space-y-4">
            ...
            <div>
                <label className="block text-xs text-koa-500 mb-1">Content</label>
                <RichTextEditor
                    value={body}
                    onChange={setBody}
                    placeholder="Write your blog post... (Markdown supported)"
                    minRows={8}
                />
            </div>
